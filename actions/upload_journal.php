...
$stmt = $pdo->prepare("INSERT INTO journals (title, authors, category, abstract, keywords, doi, volume, issue, year, published_date, file_path, cover_image, uploaded_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
if ($stmt->execute([$title, $authors, $category, $abstract, $keywords, $doi, $volume, $issue, $year, $published_date, $publicPath, $publicCoverPath, $_SESSION['user_id']])) {
