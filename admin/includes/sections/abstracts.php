...
// Patch existing tables missing the file_path column
$cols = $pdo->query("SHOW COLUMNS FROM `abstracts`")->fetchAll(PDO::FETCH_COLUMN);
            ...
            <div class="abstract-card <?= $status ?>" data-id="<?= $ab['id'] ?>"
                ...
                <div style="margin-top:10px;">
                    ...
                    <div class="abstract-full-text" style="display:none; margin-top:10px; padding:12px; background:#f8fafc; border-radius:8px; font-size:12.5px; line-height:1.7; color:#475569;">
                        ...
                        <?php if($ab['abstract_text']): ?>
                        <?= nl2br(htmlspecialchars($ab['abstract_text'])) ?>
                        <?php else: ?>
                        <span style="color:#94a3b8; font-style:italic;">No abstract text provided.</span>
                        <?php endif; ?>
                        
                        <?php if($ab['file_path']): ?>
                        <div style="margin-top:15px; padding-top:10px; border-top:1px dashed #cbd5e1;">
                            <a href="<?= htmlspecialchars($ab['file_path']) ?>" target="_blank" 
                               style="display:inline-flex; align-items:center; gap:8px; background:#3b82f6; color:#fff; padding:8px 16px; border-radius:6px; text-decoration:none; font-weight:700; font-size:12px;">
                                <i class="fas fa-file-download"></i> Download Submission Document
                            </a>
                        </div>
                        <?php endif; ?>
